...
async def send_daily_task_to_user(bot, chat_id: int):
    ...
    try:
        ...
        status_emoji = {"🟢": "📈", "🟡": "🔄", "🔴": "⚠️"}.get(user['state'], "💬")
        
        msg = (
            f"🌞 **Bonjour!** Here is today's task:\n\n"
            f"🎯 {task.task_text}\n\n"
            f"⏱️ Estimated time: {task.estimated_minutes} min\n"
...
async def handle_response(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Send feedback
    status_msg = "Level Up! ⬆️" if new_level > user['difficulty_level'] else "Keep going! 💪"
    if eval_result.adaptation_decision == 'simplify':
        status_msg = "Let's take it easy tomorrow. 🧘"
        
    reply_msg = (
        f"📝 **Évaluation:** {eval_result.score}/100\n\n"
        f"💡 **Feedback:** {eval_result.feedback}\n\n"
        f"✨ **Corrected:**\n_{eval_result.corrected_french}_\n\n"
